...
let redis;
const module_promise = Promise.resolve().then(() => __importStar(require('ioredis'))).then((m) => {
    redis = m.default;
})
    .catch(() => {
    //
...
// Adapter for Redis
// @namespace adapter
class RedisAdapter extends base_js_1.AdapterBase {
    ...
    async create(model_name, data, options) {
        ...
        try {
            if (options.use_id_in_data) {
                ...
                id = await this._client.incr(`${(0, inflector_js_1.tableize)(model_name)}:_lastid`);
            }
        }
        catch (error) {
        ...
        try {
            await this._client.hmset(`${(0, inflector_js_1.tableize)(model_name)}:${id}`, data);
        }
        catch (error) {
            throw RedisAdapter.wrapError('unknown error', error);
        }
    ...
    async update(model_name, data, options) {
        const key = `${(0, inflector_js_1.tableize)(model_name)}:${data.id}`;
        delete data.id;
        data.$_$ = ''; // ensure that there is one argument(one field) at least
        let exists;
        try {
    ...
    async updatePartial(model_name, data, conditions, options) {
        ...
        const table = (0, inflector_js_1.tableize)(model_name);
        data.$_$ = ''; // ensure that there is one argument(one field) at least
    ...
    async findById(model_name, id, options) {
        let result;
        try {
            const key = `${(0, inflector_js_1.tableize)(model_name)}:${id}`;
    ...
    async delete(model_name, conditions, options) {
        const keys = await this._getKeys((0, inflector_js_1.tableize)(model_name), conditions);
        if (keys.length === 0) {
            return 0;
        }
        let count;
