...
function _getTableNameWithSchema(model_class: typeof BaseModel): string {
  const schema = model_class.pg_schema;
  const table_name = model_class.table_name;
  return schema ? `"${schema}"."${table_name}"` : `"${table_name}"`;
}

function _processSaveError(table_name: string, error: any) {
  if (error.code === '42P01') {
...
export class PostgreSQLAdapter extends SQLAdapterBase {
  ...
  /** @internal */
  public index_name_per_table = false;
  ...
  public async create(
    ...
    const sql = this.createCommentedSQL(
      `INSERT INTO ${table_name_with_schema} (${fields}) VALUES (${places}) RETURNING id`,
      options.comment,
    );
    let result;
    try {
  ...
  public async createBulk(
    ...
    const sql = this.createCommentedSQL(
      `INSERT INTO ${table_name_with_schema} (${fields}) VALUES ${places.join(',')} RETURNING id`,
      options.comment,
    );
    let result;
    try {
  ...
  public async update(model_name: string, data: any, options: { transaction?: Transaction; comment?: string }) {
    ...
    values.push(data.id);
    const sql = this.createCommentedSQL(
      `UPDATE ${table_name_with_schema} SET ${fields} WHERE id=$${values.length}`,
      options.comment,
    );
    try {
  ...
  private async _getSchema(table: string): Promise<SchemasTable> {
    ...
    for (const column of result.rows) {
      let type: types.ColumnTypeInternal | undefined;
      if (column.data_type === 'character varying') {
        type = new types.String(column.character_maximum_length);
      ...
      } else if (column.data_type === 'vector') {
        type = new types.Vector();
