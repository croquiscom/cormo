...
class BaseModel {
  ...
  public static async _loadFromCache(key: string, refresh?: boolean): Promise<any> {
    ...
    const value = await new Promise<string | undefined>((resolve, reject) => {
      redis.get(key, (error: Error | undefined, v: string) => {
        if (error) {
          reject(error);
          return;
        }
  ...
  public static async _saveToCache(key: string, ttl: number, data: any) {
    ...
    await new Promise<void>((resolve, reject) => {
      redis.setex(key, ttl, JSON.stringify(data), (error: Error | undefined) => {
        if (error) {
          reject(error);
          return;
        }
      ...
      if (this.query_record_id_as_string) {
        for (const column of Object.keys(this._schema)) {
          ...
          if (property && property.record_id && !property.primary_key) {
            ...
            if (value) {
              if (property.array) {
                record[column] = value.map((item: any) => (item ? String(item) : null));
              } else {
        ...
        if (property && property.record_id && !property.primary_key) {
          ...
          if (value) {
            if (property.array) {
              this.set(
                column,
                value.map((item: any) => (item ? String(item) : null)),
              );
            } else {
