...
var ConnectionAssociationMixin, Promise, _, bindDomain, inflector, types;
...
ConnectionAssociationMixin = function(Base) {
  return class extends Base {
    ...
    // @private
    _applyAssociations() {
      this._pending_associations.forEach((item) => {
        ...
        return this['_' + item.type](this_model, target_model, options);
      });
    ...
    getInconsistencies(callback) {
      return this._checkSchemaApplied().then(() => {
        var promises, result;
        result = {};
        promises = Object.keys(this.models).map((model) => {
          ...
          if (integrities.length > 0) {
            return modelClass.select('').exec().then((records) => {
    ...
    fetchAssociated(records, column, select, options, callback) {
      ...
      return this._checkSchemaApplied().then(() => {
        ...
        if (association.type === 'belongsTo') {
          return this._fetchAssociatedBelongsTo(records, association.target_model, column, select, options);
        } else if (association.type === 'hasMany') {
          return this._fetchAssociatedHasMany(records, association.target_model, association.foreign_key, column, select, options);
        } else {
          return Promise.reject(new Error(`unknown column '${column}'`));
        }
