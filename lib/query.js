    ...
    exec(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if ((cache_options = this._options.cache) && (cache_key = cache_options.key)) {
                try {
                    // try cache
                    return (yield this._model._loadFromCache(cache_key, cache_options.refresh));
                }
                catch (error1) {
                    error = error1;
                    // no cache, execute query
                    records = (yield this._execAndInclude(options));
                    // save result to cache
                    yield this._model._saveToCache(cache_key, cache_options.ttl, records);
                    return records;
                }
            }
            else {
