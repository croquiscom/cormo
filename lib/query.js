...
(function() {
  ...
  Query = (function() {
    ...
    Query.prototype._validateAndBuildSaveData = function(errors, data, updates, path, object) {
      var column, error, error1, model, property, results, schema, temp;
      model = this._model;
      ...
      promises = integrities.map((function(_this) {
        return function(integrity) {
          if (integrity.type === 'parent_nullify') {
            return integrity.child.update(_.object([integrity.column], [null]), _.object([integrity.column], [ids]));
          } else if (integrity.type === 'parent_restrict') {
            return integrity.child.count(_.object([integrity.column], [ids])).then(function(count) {
              if (count > 0) {
                return Promise.reject(new Error('rejected'));
              }
            });
          } else if (integrity.type === 'parent_delete') {
            return integrity.child["delete"](_.object([integrity.column], [ids]));
          }
        };
      })(this));
