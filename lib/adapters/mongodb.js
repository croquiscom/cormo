...
(function() {
  ...
  MongoDBAdapter = (function(_super) {
    ...
    MongoDBAdapter.prototype._doIntegrityActions = function(integrities, ids, callback) {
      var _this = this;
      return async.forEach(integrities, function(integrity, callback) {
        ...
        if (integrity.type === 'parent_nullify') {
          ...
          return _this.updatePartial(integrity.child, data, conditions, {}, function(error) {
            return callback(error);
          });
        } else if (integrity.type === 'parent_restrict') {
          conditions = {};
          conditions[integrity.column] = ids;
          return _this.count(integrity.child, conditions, function(error, count) {
            if (error) {
              return callback(error);
            }
            if (count > 0) {
          ...
          conditions = {};
          conditions[integrity.column] = ids;
          return _this["delete"](integrity.child, conditions, function(error, count) {
            if (error) {
              return callback(error);
            }
            return callback(null);
    ...
    MongoDBAdapter.prototype["delete"] = function(model, conditions, callback) {
      ...
      integrities = model_class._integrities.filter(function(integrity) {
        return integrity.type.substr(0, 7) === 'parent_';
      });
      if (integrities.length > 0) {
        return async.waterfall([
          function(callback) {
            return _this._collection(model).find(conditions, {
              fields: {
                _id: 1
              }
            ...
            return _this._collection(model).remove(conditions, {
              safe: true
            }, function(error, count) {
              if (error) {
                return callback(MongoDBAdapter.wrapError('unknown error', error));
