    ...
    createIndex(model, index) {
      return new Promise((resolve, reject) => {
        ...
        return collection.ensureIndex(index.columns, options, function(error) {
          if (error) {
            return reject(MongoDBAdapter.wrapError('unknown error', error));
          } else {
            return resolve();
          }
        });
      ...
      return new Promise((resolve, reject) => {
        ...
        if (data.length > 1000) {
          ...
          async.map(chunks, (chunk, callback) => {
            return this.createBulk(model, chunk).then((ids) => {
              return callback(null, ids, (error) => {
                return callback(error);
              });
            });
            ...
            if (error) {
              ...
              return resolve(_.flatten(records_list));
            }
          });
          return;
        }
