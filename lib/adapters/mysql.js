...
var Bluebird, MySQLAdapter, SQLAdapterBase, _, _processSaveError, _propertyToSQL, _typeToSQL, e, mysql, stream, types;
...
MySQLAdapter = (function() {
  ...
  class MySQLAdapter extends SQLAdapterBase {
    ...
    async _query(sql, data) {
      ...
      return (await this._client.queryAsync(sql, data));
    }

    async _getTables() {
      var tables;
      tables = (await this._query("SHOW TABLES"));
    ...
    async _getSchema(table) {
      ...
      columns = (await this._query(`SHOW COLUMNS FROM \`${table}\``));
      schema = {};
    ...
    async _getIndexes() {
      ...
      rows = (await this._query("SELECT * FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = ? ORDER BY SEQ_IN_INDEX", [this._database]));
      indexes = {};
    ...
    async _getForeignKeys() {
      ...
      rows = (await this._query("SELECT * FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME IS NOT NULL AND CONSTRAINT_SCHEMA = ?", [this._database]));
      foreign_keys = {};
      for (j = 0, len = rows.length; j < len; j++) {
        row = rows[j];
    ...
    async drop(model) {
      ...
      try {
        await this._query(`DROP TABLE IF EXISTS \`${tableName}\``);
      } catch (error1) {
        error = error1;
        throw MySQLAdapter.wrapError('unknown error', error);
      }
    ...
    async create(model, data) {
      ...
      try {
        result = (await this._query(sql, values));
      } catch (error1) {
        error = error1;
    ...
    async findById(model, id, options) {
      ...
      if (options.explain) {
        return (await this._query(`EXPLAIN ${sql}`, id));
      }
      try {
        result = (await this._query(sql, id));
      } catch (error1) {
        error = error1;
        throw MySQLAdapter.wrapError('unknown error', error);
      }
    ...
    async find(model, conditions, options) {
      ...
      if (options.explain) {
        return (await this._query(`EXPLAIN ${sql}`, params));
      }
      try {
        result = (await this._query(sql, params));
      } catch (error1) {
        error = error1;
        throw MySQLAdapter.wrapError('unknown error', error);
      }
    ...
    async connect(settings) {
      ...
      this._client = Bluebird.promisifyAll(mysql.createPool({
        host: settings.host,
        port: settings.port,
        user: settings.user,
        password: settings.password,
