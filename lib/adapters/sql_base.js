...
SQLAdapterBase = (function() {
  ...
  class SQLAdapterBase extends AdapterBase {
    ...
    upsert(model, data, conditions, options, callback) {
      return this.updatePartial(model, data, conditions, options).then(async(count) => {
        var condition, error, i, insert_data, key, len, value;
        if (count > 0) {
        ...
        try {
          await this.create(model, insert_data);
          return callback(null);
        } catch (error1) {
          error = error1;
          ...
          return this.updatePartial(model, data, conditions, options).then((count) => {
