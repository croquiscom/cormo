...
module.exports = () ->
  ...
  it 'not startswith', (done) ->
    _createUsers _g.connection.User, (error, users) ->
      return done error if error
      _g.connection.User.where name: $not: $startswith: 'd', (error, users) ->
        return done error if error
        expect(users).to.have.length 4
        users.sort (a, b) -> if a.name < b.name then -1 else 1
        _compareUser users[0], name: 'Alice Jackson', age: 27
  ...
  it 'not endswith', (done) ->
    _createUsers _g.connection.User, (error, users) ->
      return done error if error
      _g.connection.User.where name: $not: $endswith: 'h', (error, users) ->
        return done error if error
        expect(users).to.have.length 3
        users.sort (a, b) -> if a.name < b.name then -1 else 1
        _compareUser users[0], name: 'Alice Jackson', age: 27
