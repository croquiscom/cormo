module.exports = () ->
  ...
  it 'invalid age', (done) ->
    connection.User.create { name: 'John Doe', age: 10 }, (error, user) ->
      should.exist error
      error.message.should.be.equal 'too young'
      done null

  it 'invalid email', (done) ->
    connection.User.create { name: 'John Doe', age: 27, email: 'invalid' }, (error, user) ->
      should.exist error
      error.message.should.be.equal 'invalid email'
      done null

  it 'invalid both', (done) ->
    connection.User.create { name: 'John Doe', age: 10, email: 'invalid' }, (error, user) ->
      should.exist error
